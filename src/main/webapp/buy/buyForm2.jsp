...
<div class="container" style="text-align: center">
	<form action="/project4/buy?cmd=buy" method="post" id="buyProduct">
		...
		<div class="form-group">
			...
			<input type="hidden" id="productId" value="${orders.productId }">
			<input type="hidden" id="totalPrice" value="${orders.totalPrice }">
			<input type="hidden" id="totalCount" value="${orders.totalCount }">

			<div class="d-flex">
				<div style="width: 150px; font-size: 13px; text-align: left">닉네임</div>
				<div style="font-size: 13px; text-align: left">${orders.nickName}</div>
			</div>
			...
			<div class="d-flex">
				...
				<div style="font-size: 13px; text-align: left">
					<c:set var="phone" value="${orders.phone}" />
			...
			<div class="d-flex">
				...
				<div style="font-size: 13px; text-align: left">${orders.address}</div>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						...
						<td style="padding-left: 20px; text-align: left;">
							<div style="display: flex; align-items: center;">
								<a href="/project4/product?cmd=detail&id=${orders.productId}">
									<img src="/project4/images/productImg/${orders.img}" alt="Product Image" style="width: 70px; height: auto; margin-right: 10px;">
								</a>
								<div>
									<strong>${orders.brand}</strong><br> ${orders.content}
								</div>
							</div>
						</td>
						<td><fmt:formatNumber type="number" pattern="#,##0" value="${orders.price}" />원 <br></td>
						<td><span>${orders.totalCount}개</span></td>
						<td><fmt:formatNumber type="number" pattern="#,##0" value="${orders.totalPrice}" />원 <br></td>
					</tr>
				</tbody>
			</table>
		</div>
		...
		<div class="form-group container">
			<button type="button" class="btn btn-primary btn-lg" onclick="buy()" style="width: 230px; height: 50px">
				<fmt:formatNumber type="number" pattern="#,##0" value="${orders.totalPrice}" /> 원 결제하기
			</button>
		</div>
