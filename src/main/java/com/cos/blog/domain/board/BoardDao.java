...
public class BoardDao {
	...
	*/
	...
	public int update(UpdateReqDto dto) {
		...
		Connection conn = DB.getConnection();
		PreparedStatement pstmt = null;
		System.out.println("BoardController/update/000");
		try {
			pstmt = conn.prepareStatement(sql);
			...
			int result = pstmt.executeUpdate();
			System.out.println("BoardController/update/pstmt : " + pstmt);
			System.out.println("BoardController/update/result : " + result);
			return result;
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
			System.out.println("BoardController/update/1111");
			DB.close(conn, pstmt);
		}
		return -1;
	}
	...
	// 카테고리별 게시글 목록
    public List<DetailRespDto> findByCategory(int page, int category) {
        String sql = "SELECT * FROM board b INNER JOIN user u ON b.userId = u.id WHERE category = ? ORDER BY b.id DESC LIMIT ?, 5";
        Connection conn = DB.getConnection();
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        List<DetailRespDto> boards = new ArrayList<>();
